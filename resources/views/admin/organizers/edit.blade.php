    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-black text-gray-900 mt-4 tracking-tight">Edit Akun Organizer</h1>
                <p class="text-gray-500 font-medium italic italic">Perbarui detail akun organizer: <strong>{{ $organizer->name }}</strong></p>
            </div>

            <div class="bg-white p-10 rounded-[40px] shadow-2xl shadow-indigo-100 border border-gray-100">
                <form action="{{ route('admin.organizers.update', $organizer->id) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="p-6 bg-amber-50 border border-amber-100 rounded-2xl">
                        <p class="text-amber-700 text-xs font-bold uppercase tracking-widest mb-4">Ganti Password (Opsional)</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-label for="password" :value="__('Password Baru')" class="font-bold text-gray-700 ml-1 mb-2" />
                                <input type="password" name="password" id="password" 
                                       class="block w-full px-6 py-4 bg-white border-transparent border-2 rounded-2xl focus:border-amber-500 focus:ring-0 transition-all font-medium text-gray-900 shadow-sm" 
                                       placeholder="Kosongkan jika tidak diganti">
                            </div>
                            <div>
                                <x-input-label for="password_confirmation" :value="__('Konfirmasi Password')" class="font-bold text-gray-700 ml-1 mb-2" />
